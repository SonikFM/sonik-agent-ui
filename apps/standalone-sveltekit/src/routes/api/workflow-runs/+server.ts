...
export const POST: RequestHandler = async (event) => {
  ...
  if (action.action === "run_until_blocked" || action.action === "resume_run" || action.action === "cancel_run") {
    const response = await handlePublicWorkflowDriverAction(body as PublicWorkflowDriverAction, {
      hostSession: hostSession!, store, journal: resolveWorkflowRunJournalStore(env), repository: resolveWorkflowDefinitionRepository(env),
    });
