...
import { getWorkflowHistory, WORKFLOW_HISTORY_QUERY_KEYS } from "$lib/server/workflow-history";
...
export const GET: RequestHandler = async (event) => {
  ...
  const query = Object.fromEntries(WORKFLOW_HISTORY_QUERY_KEYS.flatMap((key) => {
    const value = event.url.searchParams.get(key) ?? (key === "sessionId" ? hostSession.sessionId : null);
