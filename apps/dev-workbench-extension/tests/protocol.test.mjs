...
assert.match(serviceWorker, /const capturesInFlight = new Set<number>\(\)/, "active-tab capture state is serialized per tab");
assert.match(serviceWorker, /capturesInFlight\.has\(tabId\)[\s\S]*capturesInFlight\.add\(tabId\)/, "a concurrent capture is rejected before entering the transaction");
assert.match(serviceWorker, /catch \(error\) \{[\s\S]*pairings\.delete\(tabId\)[\s\S]*lifecycle\.revoke\(tabId\)[\s\S]*finally \{[\s\S]*capturesInFlight\.delete\(tabId\)/, "every capture failure revokes pairing and releases serialization");
