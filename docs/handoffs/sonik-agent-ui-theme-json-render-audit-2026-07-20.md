...
|---|---|---|---|
| [#2 — remaining 18 AI elements](https://github.com/SonikFM/sonik-svelte/pull/2) | **OPEN / BEHIND**; 636 changed files including committed `dist`; `unit + build + size` failed and automated review skipped the oversized source scope. Storybook smoke and consumer wiring are explicitly unrun. | **partial** — this repo has its own chat surface and JSON-render components, not this 18-family donor wave. | Salvage only a named behavior after a local gap, token, accessibility, and test review. Do not adopt its parallel schema/A2UI/Puck registry wholesale. |
...
| [#4 — Chat + EmojiPicker](https://github.com/SonikFM/sonik-svelte/pull/4) | **OPEN / UNSTABLE**; CI failed and a11y E2E was skipped. Its own description records undefined donor CSS variables and missing generated exports. | **partial** — local chat exists; the donor Chat/EmojiPicker pair is not integrated. | Reject direct copy. Product guidance bans emoji as structural chrome; salvage only an explicit user-content requirement with Sonik tokens and accessibility proof. |
| [#5 — FinalChat composer/scroll](https://github.com/SonikFM/sonik-svelte/pull/5) | **OPEN / BLOCKED**; CI failed, a11y E2E skipped, review changes requested for hidden-banner focusability plus schema/type and streaming scheduling concerns. | **partial** — local composer/conversation behaviors exist without this donor branch. | Keep blocked until the review and accessibility contract are resolved; compare behaviors, not files. |
