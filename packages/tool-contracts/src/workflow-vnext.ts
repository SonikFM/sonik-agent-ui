  ...
  definition.nodes.forEach((node, index) => {
    ...
    if (node.nodeType === "tool_commit") {
      ...
      if (!binding) issues.push({ path: ["nodes", index, "effectBinding"], code: "effect_binding_required", message: "Commit nodes require an exact effect binding" });
        ...
        if (!node.capabilityPins.length || !definition.facadeToolIds.includes(binding.commandId)) issues.push({ path: ["nodes", index], code: "capability_pin_required", message: "Commit nodes require capability and facade pins" });
