...
// so the headless harness scripts can compute "what question is next" / "are
// we ready for preview" from a json-render Spec without a Svelte runtime.
// Kept as a deliberate parallel port (not an import) because those modules
// are TS workspace-package sources that require a build step the harness
// should not depend on; the JSON Pointer paths and readiness predicate below
// are byte-for-byte the same contract the client and
// /api/artifact/[id]/state already agree on.
