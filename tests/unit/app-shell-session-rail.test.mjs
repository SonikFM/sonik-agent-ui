...
const pageContextSummarySource = await readFile("apps/standalone-sveltekit/src/lib/page-context-summary.ts", "utf8");
...
assert.equal(pageContextSummarySource.includes("CURRENT HOST/PAGE CONTEXT:"), true, "page context summary helper should inject host page context as first-class model context, not only telemetry/tool metadata");
assert.equal(pageContextSummarySource.includes("If the user asks where they are"), true, "page context summary helper should instruct page-location questions to answer from the donated page context");
...
assert.equal(commandCatalogToolsSource.includes("hostSessionInput"), true, "command catalog tools should choose trusted host session context before standalone smoke fallback");
assert.equal(commandCatalogToolsSource.includes('hostSessionMode: "standalone-demo"'), true, "command catalog tools should retain the explicit standalone demo host-session adapter for local smoke testing");
...
assert.equal(commandCatalogToolsSource.includes("sessionId: executionContext.sessionId ?? context.sessionId"), true, "command tools should preserve trusted host session id before falling back to workspace telemetry session id");
assert.equal(hostCommandRuntimeSource.includes("trusted-principal-mismatch: userId must match the trusted host principal"), true, "booking hold creation should fail closed when model input tries to override trusted principal");
