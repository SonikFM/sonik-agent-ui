...
assert.equal(generateRoute.includes("status === 400 || status === 413"), true, "trusted route/page-context failures preserve only 400/413 status");
assert.equal(generateRoute.includes('JSON.stringify({ error: status === 500 ? "Generation failed" : "Invalid request" })'), true, "generate failures expose only generic error bodies");
