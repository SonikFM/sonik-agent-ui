...
assert.match(externalEffectMigration, /jsonb_build_array\(user_id, run_id, idempotency_key, claim_id\)/i, "legacy backfill uses an unambiguous stable provenance tuple before installing org-wide uniqueness");
const legacyDigest = ({ userId, runId, idempotencyKey, claimId }) => createHash("sha256").update(JSON.stringify([userId, runId, idempotencyKey, claimId])).digest("hex");
